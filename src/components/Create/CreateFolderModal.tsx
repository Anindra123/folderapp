...
export default function CreateFolderModal() {
    ...
    const [folderColorErr, setFolderColorErr] = useState("");
    ...
    function handleSubmit() {
        ...
        if (folderColor.trim().length === 0) {
            setFolderColorErr("Please set a folder color");
            hasError = true;
        }
        else if (!COLOR_HEX_REGEX.test(folderColor)) {
            setFolderColorErr("Not a valid color hex code");
            hasError = true;
        ...
        if (!hasError) {
            CreateFolder(folderName, folderColor, folderContext)
            ...
            <dialog ref={createDialogRef} className={`bg-gray-100 border w-max backdrop:bg-gray-900 backdrop:opacity-65 border-gray-900 transition-all duration-300 ease-in-out rounded-lg ${isDialogOpen ? "scale-100" : "scale-0"}`}>
                <div className="flex flex-col rounded-lg bg-gray-100 min-w-96 p-10">
                    ...
                    <div className="w-full grid grid-cols-6 mt-10 items-center gap-y-3 gap-x-2">
                        ...
                        <div className="col-span-4">
                            <h3 className="text-red-500 text-left font-semibold text-md">{folderErr}</h3>
                        </div>
                        ...
                        <div className="col-span-4">
                            <div className="w-full rounded-lg p-3 ring-1 flex felx-row gap-x-2 ring-gray-400">
                                <input type="color" ref={folderColorRef} className="w-12 h-12 pointer-events-none" value={folderColor} onChange={(e) => setFolderColor(e.currentTarget.value)} />
                                <input type="text" className="px-3 rounded-lg bg-gray-300" value={folderColor} onChange={(e) => setFolderColor(e.currentTarget.value)} />
                        ...
                        <div className="col-span-6">
                            <h3 className="text-md text-left font-semibold text-red-500">{folderColorErr}</h3>
                        </div>
                    </div>
